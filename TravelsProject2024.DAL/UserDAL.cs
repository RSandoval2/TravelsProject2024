...
namespace TravelsProject2024.DAL
{
    public class UserDAL
    {
        ...
        private static async Task<bool> ExistsLogin(User user, ContextDB dbContext)
        {
            bool result = false;
            var userLoginExists = await dbContext.User.FirstOrDefaultAsync(u => u.Login == user.Login && u.Id != user.Id);
            if (userLoginExists != null && userLoginExists.Id > 0 && userLoginExists.Login == user.Login) { result = true; }
